            ...
            <div class="box landing-inte-back">
                ...
                <div class="box-body table-responsive-sm">
                    <table id="example2" class="table table-bordered table-hover dataTable">
                        ...
                        <tbody>
                        <?php foreach ($subtipo as $loc): ?>
                            <tr>
                                <td><?= $this->Number->format($loc->idsubtipo) ?></td>
                                <td><?= h($loc->name) ?></td>
                                <td class="actions">
                                        <?= $this->Html->link(__('Editar'), ['action' => 'edit','?' =>
                                            ['Accion' => 'Editar', 'Categoria' => 'Mapainteractive', 'Context' => 'Admin', 'id' => $loc->idsubtipo]],
                                            ['class' => 'btn btn-small btn-warning']) ?>
                                        <?= $this->Form->postLink(__('Eliminar'), ['action' => 'delete', $loc->idsubtipo],
                                            ['confirm' => __('Eliminar el registro: {0}?', $loc->idsubtipo),
